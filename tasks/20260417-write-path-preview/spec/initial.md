...
- `preview_id`: 로컬 preview 인스턴스 식별자
...
현재 phase에서 실제로 사용하는 상태는 `draft_preview`, `preview_ready`까지다.
...
- `order_preview_ready`: preview 수행에 필요한 최소 의존성이 준비됨
- `fx_preview_ready`: 환전 preview 수행에 필요한 최소 의존성이 준비됨
...
- [ ] `submit_candidate`, `warnings`, `blocking_issues` 생성 로직 구현
...
- [ ] 예상 환전 결과, warnings, blocking_issues 생성 로직 구현
...
- preview-first mutation architecture를 우선한다: `docs/write-path-opinion.md`와 `HANDOFF.md`가 모두 직접 submit보다 preview layer를 먼저 요구한다.
...
- order preview와 fx preview에 필요한 실제 endpoint 조합을 어디까지 fetch 기반으로 해결할 수 있는가.
- 시장 상태, 거래 제한, 환율/수수료 정보 중 브라우저 fetch로 안정적으로 얻기 어려운 값은 무엇인가.
- preview fingerprint가 시간 민감 필드까지 포함해야 하는지, 아니면 정규화 입력과 핵심 파생 값만 포함해야 하는지.
- preview 결과를 메모리만으로 유지할지, 런타임 홈에 짧게 저장할지.
- `post_submit_verify_ready`를 health capability에 이번 phase부터 넣을지, submit spec에서만 추가할지.
- order preview의 `submit_candidate`가 실제 broker submit payload와 어느 정도까지 동일해야 하는지.
- FX preview에서 수취 예상 금액 계산에 필요한 수수료/환율 정보를 어떤 endpoint로 확보할지.
